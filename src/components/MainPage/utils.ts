...
export function mapCubeUV_v1(geometry: THREE.BufferGeometry, cubeH: number = 0): THREE.BufferGeometry {
    ...
    for (let i = 0; i < indexes.length; i += 3) {
        ...
        for (let j = 0; j < ids.length; j++) {
            ...
            const vertex = new THREE.Vector3(position[vertexIndex], position[vertexIndex + 1], position[vertexIndex + 1]);
            let tileIx;
            let uvY;
            let uvX;
            switch(faceSign) {
...
export function mapCubeUV_v2(geometry, cubeH) {
    ...
    for (let i = 0; i < indexes.length; i += 3) {
        ...
        for (let j = 0; j < ids.length; j++) {
            ...
            const vertex = new THREE.Vector3(position[vertexIndex * 3], position[vertexIndex * 3 + 1], position[vertexIndex * 3 + 1]);
            let tileIx = 0;
            let uvY = 0;
            let uvX = 0;
            switch(faceSign) {
