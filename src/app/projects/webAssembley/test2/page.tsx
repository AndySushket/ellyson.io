...
// Import the WebAssembly module loader
...
export default class WebAssemblySphere extends TemplateFor3D {
  ...
  async loadWasmModule() {
    try {
      // Load the WebAssembly module using npm approach
      this.wasmModule = await loadSphereModule();

      this.setState({ wasmLoaded: true });
      console.log('WebAssembly module loaded successfully using npm');
      ...
      // Fallback to the traditional approach if npm loading fails
      try {
        console.log('Falling back to traditional WebAssembly loading...');
        await this.loadWasmModuleFallback();
      } catch (fallbackError) {
  ...
  // Fallback method to load WebAssembly the traditional way
  async loadWasmModuleFallback() {
    // Load the WebAssembly module using script tag
    ...
    const modulePromise = (window as any).createSphereModule();
    this.wasmModule = await modulePromise;

    this.setState({ wasmLoaded: true });
    console.log('WebAssembly module loaded successfully via fallback');
